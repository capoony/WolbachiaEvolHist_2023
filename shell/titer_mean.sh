...
while read -r ID; do
    ...
    samtools depth $i |
        python ${PWD}/scripts/mean_depth.py \
            --input - |
        awk -v ID=$ID 'NR>1{print ID"\t"$0}' >${PWD}/results/titer/Full_coverages_${ID}_mean.txt &

done <${PWD}/datasets/AllSamples.txt

wait

cat ${PWD}/results/titer/Full_coverages_*_mean.txt |
    awk '{print $1"\t"$2"\tNA\tNA\tNA\tNA\tNA\t"$3"\tNA\tNA"}' \
        >${PWD}/results/titer/Full_coverages_mean.txt

python ${PWD}/scripts/SumReadDepths_median.py \
    --input ${PWD}/results/titer/Full_coverages.txt \
    --median ${PWD}/results/titer/Full_coverages_mean.txt \
    --names ${PWD}/data/names.txt \
    --status ${PWD}/data_old/OldStuff/output/ReadDepths/InfStatus.txt \
    >${PWD}/results/titer/Full_coverages_mean.summary

echo """

library(tidyverse)
library(scales)
DATA=read.table('${PWD}/results/titer/Full_coverages_mean.summary',
    header=T)
...
ggsave('${PWD}/results/titer/Full_coverages_full_mean2.pdf',
    PLOT,
    width=10,
    height=6)

ggsave('${PWD}/results/titer/Full_coverages_full_mean2.png',
    PLOT,
    width=10,
    height=6)

""" >${PWD}/results/titer/Full_coverages_mean2.r

Rscript ${PWD}/results/titer/Full_coverages_mean2.r
